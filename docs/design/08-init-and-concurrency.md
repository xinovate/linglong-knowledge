...
```mermaid
flowchart TD
    Start([用户执行 linglong kb init]) --> Check{"已有<br/>~/knowledge/?"}

    Check -->|"是"| Found["检测已有数据<br/>wiki/ + db/ + .knowledge.yml"]
...
| 步骤 | 操作 | 产物 |
|------|------|------|
| 1. 创建目录 | `~/knowledge/wiki/` 6 个 facet 子目录 + `archive/` | 目录结构 |
| 2. 生成配置 | 交互式问答 → `.knowledge.yml` | 配置文件 |
| 3. 创建索引文件 | `index.md` + `index-*.md` + `log.md` | 索引骨架 |
| 4. 初始化数据库 | `~/knowledge/db/knowledge.db`（空表结构） | SQLite |
...
| 步骤 | 操作 |
|------|------|
| 1. 克隆仓库 | `git clone <repo> ~/knowledge/wiki/` |
| 2. 检测配置 | 如果仓库中有 `.knowledge.yml`，直接使用；否则交互式生成 |
...
| 步骤 | 操作 |
|------|------|
| 1. 复制 wiki 目录 | `cp -r <backup>/wiki/ ~/knowledge/wiki/` |
| 2. 复制配置 | `cp <backup>/.knowledge.yml ~/knowledge/` |
| 3. 重建索引 | 从 wiki 文件重建 SQLite + 向量索引 |
...
✅ 配置已生成：~/knowledge/.knowledge.yml
✅ 目录已创建：~/knowledge/wiki/ (6 facet + archive)
✅ 数据库已初始化：~/knowledge/db/knowledge.db
✅ embedding 服务连通性：可用
✅ 索引已构建：0 条知识
...
✅ 配置文件：~/knowledge/.knowledge.yml
✅ SQLite 数据库：~/knowledge/db/knowledge.db (WAL mode)
...
LOCK_FILE = "~/knowledge/.knowledge.lock"
...
| 编号 | 决策 | 选择 | 原因 | 替代方案 |
|------|------|------|------|----------|
| D-08a | 默认存储路径 | ~/knowledge/ | 跨项目共享，防止覆盖项目配置 | 项目本地 ./knowledge/ |
...
| v1.1 | 2026-05-18 | 默认路径改为 ~/knowledge/，配置模板同步更新 | Part A 初始化 |
