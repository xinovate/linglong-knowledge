...
**负责会话**: **主会话（推荐）** 或子 Agent（由 Architect dispatch）
...
| 方式 | 适用场景 | 风险 |
|------|---------|------|
| **主会话直接审查（推荐）** | 常规功能实现、已有上下文 | 无 |
| dispatch Reviewer 子 Agent | 大型重构、需要独立视角 | **子 Agent 可能陷入文件读取循环** |

> **经验**: 在 OpenClawSyncAdapter 和 ClaudeCodeSyncAdapter 的审查中，Spec Reviewer 和 Code Quality Reviewer 子 Agent 均出现了反复读取同一批参考文件的异常循环（数十次重复读取 `openclaw.py`、`test_openclaw_sync.py` 等文件，无输出）。因此**默认推荐主会话直接审查**，仅在需要完全独立视角时才考虑 dispatch Reviewer 子 Agent。
...
### 4.1 两阶段审查（推荐：主会话直接完成）

**默认流程**（主会话直接审查，避免子 Agent 循环风险）：
...
Developer 完成实现
        ...
        ▼
┌──────────────────────────┐
│  Architect 直接审查       │  ← 本会话读取代码 + make check
│  ① Spec Review            │     对照计划检查功能完整性
│  ② Code Quality Review    │     对照检查清单检查代码质量
└───────────┬──────────────┘
            │
       ✅ 通过 │ ❌ 不通过（Developer 修复后重新审查）
            │
            ▼
       Architect 合并
```

**替代流程**（dispatch Reviewer 子 Agent，仅在需要完全独立视角时使用）：
...
Developer 完成实现
        ...
        ▼
┌─────────────────┐
│  Spec Reviewer  │  ← 子 Agent 审查（⚠️ 有循环风险）
│   规格审查       │
└────────┬────────┘
         │
    ✅ 通过 │ ❌ 不通过
         ...
         ▼
┌─────────────────┐
│ Quality Reviewer│  ← 子 Agent 审查（⚠️ 有循环风险）
│   代码质量审查   │
└────────┬────────┘
         │
    ✅ 通过 │ ❌ 不通过
...
**审查方式决策**:
- **常规实现**（1-3 个文件、已有清晰计划）→ **主会话直接审查**
- **大型重构**（跨模块、需要独立视角）→ 可考虑 dispatch Reviewer 子 Agent，但需准备好替代方案（主会话兜底）
