...
> 创建日期：2026-05-25 | 最后更新：2026-05-26 | 状态：已实现
...
| D-06 | [MCP 接入](06-mcp.md) | 接入层 | ✅ 已实现 | D-02 | 2026-05-26 |
...
| GD-08 | 模块工具控制 | enabled_modules 配置 | 部署时只暴露需要的工具 | 全量暴露 |
...
所有数据源、关键词、去重窗口、缓存策略通过 `.linglong.yaml` 管理，无需改代码。
...
| v2.6 | 远程部署 | HTTP 传输 + Token 认证 + 模块工具控制 |
...
| `BriefHistory` | `ingest/brief_history.py` | 按维度跨天去重 + 重叠检测 + fallback |
| `SourcePackage` | `ingest/package.py` | 采集包定义模型 |
| `FeedbackStore` | `ingest/feedback.py` | 用户偏好存储 + 权重计算 |
| `morning_brief.md` | `ingest/prompts/morning_brief.md` | 早报 prompt 模板 |

---

## 外部配置

所有配置通过 `~/.linglong.yaml` 管理：
...
| LLM | `reviewer.llm_model` / `reviewer.llm_base_url` | glm-5.1 + Anthropic 端点 |
| MCP | `mcp.transport` / `mcp.auth_token` / `mcp.enabled_modules` | 传输方式 + 认证 + 工具控制 |
...
| Phase 16 | HTTP 传输 + Token 认证 + 模块工具控制 | ✅ v2.6 |
| Phase 17 | 关键词质量优化 + 信源可信度权重 | 🔜 规划中 |
