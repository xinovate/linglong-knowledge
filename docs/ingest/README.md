...
PackageExecutor 执行 8 个阶段：

1. **Fetch legacy** — 采集旧格式数据源
2. **Fetch dimensions** — 按维度并行搜索
3. **Verification** — 5 层真实性验证
4. **Dedup** — 跨天去重（内容哈希 + 标题关键词重叠）
5. **Filter** — 维度过滤（max_results + max_age_days）
6. **LLM interpret** — 按维度批量解读 + Top 5 精选
7. **Persist** — 写入 ingest_history
8. **Format** — 模板格式化输出
