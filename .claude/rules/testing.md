...
class TestSearchHybrid:
    def test_returns_matching_entities(self, store_with_data):
        ...

    def test_returns_empty_for_no_match(self, empty_store):
        ...
```
...
# 坏：mock 内部业务函数
@patch("linglong.ingest.agent.IngestAgent._build_prompt")
...
- fixture 命名表达内容：`store_with_data`、`empty_store`、`mock_llm_client`
...
def store_with_data(tmp_path):
    ...
    store.add_entity(Entity(name="test", facet="concept"))
    yield store
    store.close()
```
