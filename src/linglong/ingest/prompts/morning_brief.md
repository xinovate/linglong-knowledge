...
10. **开源趋势维度优先使用 GitHub 数据区的精确数据**：项目名、Stars 数、链接必须使用 GitHub 数据区的数据，不要编造。表格为"项目名 | 分类 | Stars | 解读 | 链接"五列格式，分类列填写"日增长 #1""周增长 #1""月增长 #1"等，不要把分类写在项目名里。按日增长、周增长、月增长顺序排列，每段按增长量降序。不要在表格下方单独加数据来源注释
