...
class HexoPublisher(Publisher):
    ...
    def health_check(self) -> bool:
        ...
        if self.use_git_workflow:
            ...
            result = subprocess.run(
                ['git', '-C', str(self.hexo_path), 'rev-parse', '--git-dir'],
                capture_output=True
            )
    ...
    def _git_publish(self, file_path: Path, metadata: Dict[str, Any], filename: str) -> PublishResult:
        """Git 工作流：add → commit → push"""
        try:
            ...
            if push_result.returncode != 0:
                return PublishResult(
                    success=False,
                    error=f"git push 失败: {push_result.stderr}"
                )
    ...
    def _local_publish(self, file_path: Path, metadata: Dict[str, Any], filename: str) -> PublishResult:
        """Local 工作流：hexo generate → deploy"""
        try:
            # 执行 hexo generate
            result = subprocess.run(
            ...
            if result.returncode != 0:
                return PublishResult(
                    success=False,
                    error=f"hexo generate 失败: {result.stderr}"
                )
            ...
            if self.auto_deploy:
                ...
                if deploy_result.returncode != 0:
                    return PublishResult(
                        success=False,
                        error=f"hexo deploy 失败: {deploy_result.stderr}"
                    )
